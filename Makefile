...
build_macos_app:
	@echo "Building Macos..."
	gogio -ldflags="-X main.appVersion=$(TAG_NAME)" -appid=rest.chapar.app -icon=./build/appicon.png -target=macos -arch=amd64 -o ./dist/amd64/Chapar.app .
	gogio -ldflags="-X main.appVersion=$(TAG_NAME)" -appid=rest.chapar.app -icon=./build/appicon.png -target=macos -arch=arm64 -o ./dist/arm64/Chapar.app .
...
build_windows:
	...
	gogio -ldflags="-X main.appVersion=${TAG_NAME}" -target=windows -arch=amd64 -o "dist\amd64\Chapar.exe" .
	gogio -ldflags="-X main.appVersion=${TAG_NAME}" -target=windows -arch=386 -o "dist\i386\Chapar.exe" .
	gogio -ldflags="-X main.appVersion=${TAG_NAME}" -target=windows -arch=arm64 -o "dist\arm64\Chapar.exe" .
	rm *.syso
...
build_linux:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -ldflags="-X main.appVersion=$(TAG_NAME)" -o ./dist/amd64/chapar .
